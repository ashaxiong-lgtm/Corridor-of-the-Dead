...
// Camera/player er position (x,y,z) ar kon dike takay ache (yaw = bam-dan, pitch = upor-niche)
static float camX = 0.0f, camY = 1.7f, camZ = 10.0f;
static float yaw = -90.0f, pitch = 0.0f;
//কোন কোন key চাপা আছে তা track করার array
static int keyState[256], specialState[256];
// warpingPointer: mouse ke center e ferot anle je fake motion event ashe seta ignore korar flag
// mouseFire: left click dhore rakha ache kina (auto fire er jonno)
static int warpingPointer = 0, mouseFire = 0;
static const float PLAYER_R = 0.34f;

/* ---- combat / gun state ---- */
// Tracer = guli chhorar por je alor rekha dekha jay; shuru (x1..) ar shesh (x2..) point, ar koto shomoy dhore ache (age)
struct Tracer { float x1, y1, z1, x2, y2, z2, age; };
static std::vector<Tracer> tracers;
static const float TRACER_LIFE = 0.08f;
// Player er health, kotogula zombie marse, koto guli chhorse, ar shesh guli kokhon chhorse (ms e)
...
/* ---- multi-floor lift progression ----
   Floors need 4/5/6 kills. liftState: 0 = locked (quota not met),
...
// Door er data: position, HUD e dekhano naam, kon type er door, ar kotota ghurse (angle, degree e)
typedef struct {
    float x, y, z;
    char label[48];
    int type;       /* 0 room, 1 elevator, 2 washroom, 3 washroom cubicle */
    float angle;
} Door;

// Shob door er list; doorTarget[i] = 1 mane door ta khulte chay, 0 mane bondho hote chay
static Door doors[MAX_DOORS];
static int doorCount = 0, nearestDoor = -1;
static int doorTarget[MAX_DOORS];

// Rect = axis-aligned ayotokhetro (x ar z e min/max); wall ar furniture er collision er jonno use hoy
typedef struct { float minx, maxx, minz, maxz; } Rect;
static Rect walls[128], obstacles[256];
static int wallCount = 0, obstacleCount = 0;

// Texture banano r jonno temporary pixel buffer (RGB), ar banano texture gular OpenGL id
static GLubyte texBuf[TEXSZ * TEXSZ * 3];
static GLuint tileTex, woodTex, metalTex, wallTex, sceneTex;

// 4 ta classroom er center er x position (corridor er dui pashei eki x)
...
// List e notun ekta Rect add kore; list full (cap) hoye gele chup chap kichu kore na
...
// Point (x,z) ta rectangle er bhitore ache kina check kore
...
// Player er circle ta list er kono ekta rect er sathe dhakka khay kina
...
// Lift door er alada rule; baki door er pata ke hinge er local space e ghuriye niye check kora hoy
...
// Main collision check: wall, door ba furniture - jekono ekta te lagle 1 return kore.
// Player chalanor shomoy (movePlayer) eta diye check kora hoy notun jaygay jawa jabe kina
static int collides(float x, float z) {
    return collidesStatic(x, z) || collidesDoors(x, z) || collidesFurniture(x, z);
}

// Notun door add kore; shuru te shob door bondho thake (angle = 0)
...
// clampf: value ke lo..hi er moddhe atkay rakhe; frand: 0..1 er moddhe random number
static float clampf(float v, float lo, float hi) { return fmaxf(lo, fminf(hi, v)); }
static float frand(void) { return (float)rand() / (float)RAND_MAX; }

// texBuf e ekta pixel er RGB set kore; texture er baire gele ignore kore
...
// Texture er moddhe ekta solid color er rectangle bhore dey
...
/* Integer hash -> 0..1. same pattern pawar jonno */
...
/* Smooth value noise with `cells`, smoth pattern ready kore, 
fbm= detailed texture
   TEXSZ s the textures still tile seamlessly. */
...
// texBuf er pixel gula GPU te pathay: repeat wrap ar mipmap shoho notun texture banay
static void uploadTex(GLuint* id) {
    ...
    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, TEXSZ, TEXSZ, GL_RGB, GL_UNSIGNED_BYTE, texBuf);   //full texture code ei generate kora
}
...
// Shob procedural texture ekhane code diye banano hoy - floor, kath, metal, deyal ar janalar bairer drishyo.
// Kono image file lage na; noise (fbm/vnoise) ar hash diye purono-noshto look ta ana hoy
static void genTextures(void) {
    ...
    // Floor e 10 ta random-walk fatol (crack) aka hoy
    for (i = 0; i < 10; i++) {
    ...
    // Dur er kalo pahar er silhouette (duita sine wave mishiye)
    for (x = 0; x < TEXSZ; x++) {
        int top = (int)(136 - 9 * sinf(x * 2 * PI / TEXSZ * 2 + 1) - 5 * sinf(x * 2 * PI / TEXSZ * 5));
        for (y = top; y < 150; y++) setPx(x, y, 11, 14, 17);
    }
    // Majhkhane matir rasta, kache (niche) ashle chowra hoy - perspective er moto
    for (y = 150; y < TEXSZ; y++) {
    ...
    // 7 ta kobor: kichu cross, baki gula gol-matha pathor
    {
    ...
    // Tin ta pata-chhara mora gach
    texBranch(22, 175, 24, PI / 2, 5);
    texBranch(236, 168, 28, PI / 2 + .1f, 5);
    texBranch(140, 150, 12, PI / 2 - .1f, 4);
    // Mathir upor kuasha (mist) er ekta band halka kore blend kora
...
/* Drawing helpers                                               */
/* ------------------------------------------------------------ */
// phong lighting model 35% intensity
// Material er ambient, diffuse, specular set kore; ambient = color er 35%, halka specular chokchoke bhab
...
// Center (x,y,z) e w*h*d size er solid box aka - pura game er geometry er main building block
static void box(float x, float y, float z, float w, float h, float d) {
    glPushMatrix();   // matrix save kora hoy basically translate kore then scale kore
    glTranslatef(x, y, z);
    glScalef(w, h, d);
    glutSolidCube(1);
    glPopMatrix();  // matrix restore kora hoy
}
...
// 6 ta face er normal, vertex ar texture coordinate table theke nijei quad aka hoy
...
// Mathite ekta halka kalo oval (fake shadow) aka, transparency blend diye
...
// printf er moto format kore screen e text likhe (HUD er jonno)
...
/* Brightness of ceiling tube `id`: some are dead, the rest hum and now and
   then stutter out. */
// ~22% tube ekdom nishe (dead); baki gula sine wave e halka jhikimiki kore, majhe majhe hotat kome jay
...
// Game shuru te ekbar cholbe: shob wall, furniture obstacle ar door er collision table banano hoy
...
// Floor e tile texture, deyal er niche baseboard, chad, ar chhader tube light gula
...
// Collision table er protita wall ke deyal er texture diye 3m uchu box hishebe aka;
// patla wall gula ke 0.32 mota kore dekhano hoy
...
// Toilet cubicle er door: bam pashe hinge, angle onujayi cubicle er bhitore ghure
...
// Classroom/washroom er kather door: fixed frame + hinge e ghora pata (leaf)
...
// Ekta classroom aka: chalkboard, teacher desk, 9 ta purono hospital bed, lamp ar 4 ta fan.
// south = corridor er dokkhin pasher room (tokhon shob kichu ulto dike)
...
// Zombie er cylinder/disk akar jonno GLU quadric, ar zombie animation er ghori
static GLUquadric* zombieQuad = NULL;
static float zombieAnimTime = 0.0f;

// Protiti zombie er data: position, kon zone e ghore, patrol er kon point e jacche, health, jibito/mortese
typedef struct {
    float x, z;
...
// 6 ta zombie er shuru r position, zone, path index, animation phase ar health (3 guli lage)
...
// Zombie ei jaygay jete parbe na jodi: wall/door/furniture e lage, map er baire jay,
// ba (room er zombie hole) nijer room er baire chole jete chay
...
// Porer patrol point e jao (4 ta point ghure ghure)
static void nextWaypoint(SEUZombieNPC* z) { z->pathIndex = (z->pathIndex + 1) % 4; }

// Protiti frame e zombie AI update: mora hole respawn timer,
// player kache thakle taake dhawa, noile nijer patrol route e hata
static void updateSEUZombies(void) {
    for (int i = 0; i < ZOMBIE_COUNT; i++) {
        ...
        // Player 6.5m er moddhe thakle soja taar dike dour; x ar z alada check jate deyal e atke na jay
        ...
        // Noile patrol: target waypoint e pouchale porer ta nao
        float tx, tz;
...
// Ekta angul: metal er dondo + agay kalo cone (nokh); droop = koto ta niche bhanga
...
// Hater talu + 4 ta nokh-wala angul + ekta buro angul
...
// Robot-zombie er matha: box shell, lal jolonto chokh (emission), mukher grille, dat ar 4 ta antenna
...
// Shorir (buk): box + nicher plate er seam + dag
...
// Hat: upore bahu, niche forearm, tarpor hand - hierarchical transform e ekta arekta r niche jure thake
...
// Hierarchical model: hip -> dui pa, torso -> ghar -> matha, torso -> dui hat.
// stride diye pa samne-pichone dole, t diye hat halka nore
...
// Shob zombie ke world e bosano: kon dike takabe (heading) hishab kore;
// mortese (state 1) hole pichone pore dhire dhire niche dube jay
...
// Color set kore ekta box aka - bonduker part akar shortcut
...
// bobX/bobY = hatar shomoy bonduk halka upor-niche/dane-bame dole
...
// Ray-sphere intersection: b ar disc (discriminant) diye quadratic solve kore
// ray ta shobcheye kache kon zombie ke lage seta ber kora hoy
static void tryShoot(void) {
    ...
    // Lagle health komao; 0 hole zombie more, kill bare, quota purno hole lift khule ba game jeto
...
// Pura game notun kore shuru (R chaple): floor 1, full health, shob count 0
...
// Porer floor e jao ar 'FLOOR N' banner 2.5 second dekhao
...
// Corridor er matha, classroom er kona ar washroom e makorshar jal bosano; hash diye kichu kona faka rakha hoy
static void drawCobwebs(void) {
    glDisable(GL_LIGHTING);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); //standard transparency blending shadow
    glLineWidth(1.0f);
...
// Mombati gulor position (x, y, z) - mathite ba desk/counter er upor
...
// Dui ta sine wave mishiye mombatir alor jhikimiki (mote 0.64..1.0)
static float candleFlicker(int i) {
    return .82f + .10f * sinf(hauntTime * 23.0f + i * 3.1f) + .08f * sinf(hauntTime * 37.0f + i);
}

// Mombatir gola mom, main mombati, choto mombati ar shalita (wick) aka
...
// Player er shobcheye kacher mombati khuje ber kore - oi ta LIGHT3 hishebe alo dey
...
// Player er charpashe 7m er moddhe random jaygay dhulo shuru kora
...
// Dhulo halka bhese bhese niche name; player theke 7m er beshi dure gele ulto pashe wrap hoye ashe
...
// Dhulo gula choto transparent point hishebe aka
...
static void drawOldFilmOverlay(void) {
    ...
    glBlendFunc(GL_DST_COLOR, GL_ZERO); /* multiply blend — old film tint */
...
// Shob opaque jinish aka: building, deyal, room, washroom, door, lift, janala, mombati, deyal er lekha ar zombie
...
// Player er 3.4m er moddhe shobcheye kacher door ta nearestDoor e rakho (na thakle -1)
...
static void animateDoors(void) {
    for (int i = 0; i < doorCount; i++) {
        ...
        float target = doorTarget[i] ? (doors[i].type == 3 ? -78.0f : 78.0f) : 0.0f;// dorjar angle
...
// WASD/arrow onujayi player ke nara; Shift (keyState['j'] == 2) chaple dour.
// x ar z alada check kore, tai deyal e lagle deyal ghese slide kora jay
...
// Key chaple: ESC = ber hoye jao, R = restart, E = door, O = lift, baki gula keyState e mark
...
// Key chhere dile keyState 0 kore dao
static void keyboardUp(unsigned char key, int x, int y) {
    (void)x; (void)y;
    keyState[(unsigned char)tolower(key)] = 0;
}

// Arrow key er moto special key er down/up track kora
...
// Mouse center theke koto sorlo tar upor yaw/pitch bare; pore abar center e warp kora hoy (FPS style look)
...
static void drawLiftCompass(void) {
    ...
    // Lift er dik theke player er yaw bad diye relative angle, -180..180 er moddhe ana
...
// Upore controls/stats, niche door er message, majhe crosshair, ar game over/win lekha
...
// Protiti frame render: 3D projection + camera, alo update, scene, tracer, bonduk, tarpor 2D HUD/film overlay
...
// game loop player move, zombie AI
...
// Ekta light on kore tar diffuse, ambient, specular color set kore
...
// OpenGL er shuru r setting: depth, lighting, fog, spotlight; ar texture, geometry, dhulo ekbar banano
static void initGL(void) {
    ...
    setupLight(GL_LIGHT0, diff0, amb0, spec0); // Point light 1 (flickering ceiling)
    setupLight(GL_LIGHT1, diff1, amb1, spec1);   // Point light 2 (cool fill)
    setupLight(GL_LIGHT2, diff2, amb2, spec2);   // Spotlight (flashlight on gun)
    setupLight(GL_LIGHT3, diff3, amb3, spec3); // Candle point light
    ...
    glLightf(GL_LIGHT2, GL_SPOT_CUTOFF, 22.0f);  // alo r kon kmn
...
// Program shuru: GLUT window banano, shob callback register, tarpor main loop cholte thake
int main(int argc, char** argv) {
    glutInit(&argc, argv);
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH);
    glutInitWindowSize(WIN_W, WIN_H);
    glutCreateWindow(" Corridor Explorer - Haunted Night");
